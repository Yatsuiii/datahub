...
public class IngestionCliVersionMatrixServiceFactory {
  ...
  // otherwise, in which case an s3:// URI degrades to a no-op matrix source with a warning rather
  // than failing startup. Reused rather than building a second S3 client so the matrix inherits the
  // same role assumption / endpoint / region resolution as every other S3 caller in GMS.
    ...
    try {
      ...
      return switch (location.provider()) {
        case S3 -> {
          if (s3Util == null) {
            log.warn(
                "ingestion.cliVersionMatrix.uri is {} but no s3Util bean is available "
                    + "(set AWS_REGION or datahub.s3.roleArn); matrix lookups disabled.",
                uri);
            yield new NoOpIngestionCliVersionMatrixSource();
          }
          yield matrixSource(
              new S3ObjectStorageClient(s3Util.getS3Client(), location.bucket(), null),
              location.keyPrefix(),
              uri,
              refreshSeconds);
        }
