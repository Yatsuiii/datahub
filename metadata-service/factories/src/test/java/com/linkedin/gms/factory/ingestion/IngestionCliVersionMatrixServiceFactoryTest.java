...
public class IngestionCliVersionMatrixServiceFactoryTest {
  ...
  public void testMatrixSource_whenUriIsS3AndClientAvailable_wiresObjectStorageSource() {
    ...
    setField(factory, "s3Util", s3UtilWithClient());
  ...
  public void testMatrixSource_whenS3ClientUnavailable_wiresNoOp() {
    // s3:// but no s3Util bean (AWS not configured) → application default, not a startup failure.
    ingestionConfig.getCliVersionMatrix().setUri("s3://cli-version-matrix/matrix.json");
  ...
  /**
   * The factory only needs the wrapped client, which S3ObjectStorageClient stores without calling.
   */
  private static S3Util s3UtilWithClient() {
    S3Util s3Util = mock(S3Util.class);
    when(s3Util.getS3Client()).thenReturn(mock(S3Client.class));
    return s3Util;
  }
